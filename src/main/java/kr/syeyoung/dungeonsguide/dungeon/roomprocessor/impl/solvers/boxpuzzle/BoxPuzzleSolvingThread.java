...
package kr.syeyoung.dungeonsguide.dungeon.roomprocessor.impl.solvers.boxpuzzle;
...
public class BoxPuzzleSolvingThread extends Thread {
    ...
    public static boolean push(byte[][] board, int x, int y, int dx, int dy) {
        ...
        int resultingX = x + dx;
        int resultingY = y + dy;
        if (resultingX < 0 || resultingY < 0 || resultingX >= board[0].length || resultingY >= board.length)
            return false;
