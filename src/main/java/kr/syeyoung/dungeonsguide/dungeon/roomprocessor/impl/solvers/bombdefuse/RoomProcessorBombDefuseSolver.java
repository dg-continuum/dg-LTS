...
package kr.syeyoung.dungeonsguide.dungeon.roomprocessor.impl.solvers.bombdefuse;
...
import kr.syeyoung.dungeonsguide.dungeon.roomprocessor.impl.solvers.bombdefuse.chambers.BombDefuseChamberGenerator;
import kr.syeyoung.dungeonsguide.dungeon.roomprocessor.impl.solvers.bombdefuse.chambers.DummyDefuseChamberProcessor;
import kr.syeyoung.dungeonsguide.dungeon.roomprocessor.impl.solvers.bombdefuse.chambers.arrow.ArrowProcessorMatcher;
import kr.syeyoung.dungeonsguide.dungeon.roomprocessor.impl.solvers.bombdefuse.chambers.bugged.ImpossibleMatcher;
import kr.syeyoung.dungeonsguide.dungeon.roomprocessor.impl.solvers.bombdefuse.chambers.color.ColorProcessorMatcher;
import kr.syeyoung.dungeonsguide.dungeon.roomprocessor.impl.solvers.bombdefuse.chambers.creeper.CreeperProcessorMatcher;
import kr.syeyoung.dungeonsguide.dungeon.roomprocessor.impl.solvers.bombdefuse.chambers.goldenpath.GoldenPathProcessorMatcher;
import kr.syeyoung.dungeonsguide.dungeon.roomprocessor.impl.solvers.bombdefuse.chambers.maze.MazeProcessorMatcher;
import kr.syeyoung.dungeonsguide.dungeon.roomprocessor.impl.solvers.bombdefuse.chambers.number.NumberProcessorMatcher;
...
public class RoomProcessorBombDefuseSolver extends GeneralRoomProcessor {
    ...
    public RoomProcessorBombDefuseSolver(DungeonRoom dungeonRoom) {
        super(dungeonRoom);
        if (!DgOneCongifConfig.bombDefuseSolver) {
            bugged = true;
            return;
        }
        ...
        if (warning1 != null) warning = warning1.getVector3i(dungeonRoom);
    }
    ...
    public void tick() {
        ...
        OffsetPoint offsetPoint = new OffsetPoint(getDungeonRoom(), new Vector3i(player.getX(), 68, player.getZ()));
        for (ChamberSet ch : chambers) {
            if (ch.getLeft() != null && ch.getLeft().getProcessor() != null) {
    ...
    public void drawWorld(float partialTicks) {
        ...
        OffsetPoint offsetPoint = new OffsetPoint(getDungeonRoom(), new Vector3i((int) Minecraft.getMinecraft().thePlayer.posX, 68, (int) Minecraft.getMinecraft().thePlayer.posZ));
        boolean found = false;
        for (ChamberSet ch : chambers) {
            if (ch.getLeft() != null && ch.getLeft().getProcessor() != null) {
                if (ch.getLeft().getChamberBlocks().getOffsetPointList().contains(offsetPoint)) {
        ...
        if ((maze || impossible) && warning != null && !found) {
            if (impossible) {
                RenderUtils.drawTextAtWorld("Warning: This Bomb Defuse is bugged and Impossible", warning.x + 0.5f, warning.y, warning.z + 0.5f, 0xFF00FF00, 0.03F, false, false, partialTicks);
            } else {
                RenderUtils.drawTextAtWorld("Warning: This Bomb Defuse must be done with 2 people (maze)", warning.x + 0.5f, warning.y, warning.z + 0.5f, 0xFF00FF00, 0.03F, false, false, partialTicks);
            }
        }
        if (warning != null && !found) {
            for (int i = 0; i < 4; i++) {
                BombDefuseChamberGenerator bdcg = chambers.get(i).getChamberGen();
                RenderUtils.drawTextAtWorld((i + 1) + ". " + (bdcg == null ? "null" : bdcg.getName()), warning.x + 0.5f, warning.y - ((i + 1) * 0.3f), warning.z + 0.5f, 0xFF00FF00, 0.03F, false, false, partialTicks);
            }
        }
    }
