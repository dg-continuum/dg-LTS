...
public class RoomProcessorBlazeSolver extends GeneralRoomProcessor {
    ...
    public void tick() {
        ...
        final Vector3i low = new Vector3i(dungeonRoom.getMin());
        final Vector3i high = new Vector3i(dungeonRoom.getMax());
        entityList = new ArrayList<>(w.getEntities(EntityArmorStand.class, input -> {
        ...
        blazeList = new ArrayList<>(w.getEntities(EntityBlaze.class, input -> {
            BlockPos pos = input.getPosition();
            return low.x < pos.getX() && pos.getX() < high.x
