...
public class RoomProcessorTeleportMazeSolver extends GeneralRoomProcessor {
    ...
    public void drawWorld(float partialTicks) {
        ...
        for (BlockPos bpos : visitedPortals) {
            RenderUtils.highlightBoxAColor(AxisAlignedBB.fromBounds(bpos.getX(), bpos.getY(), bpos.getZ(), bpos.getX() + 1, bpos.getY() + 1, bpos.getZ() + 1), DgOneCongifConfig.oneconftodgcolor(TeleportSolverPage.nonSolutionColor), partialTicks, true);
        }

        if (intersection != null) {
            RenderUtils.highlightBoxAColor(AxisAlignedBB.fromBounds(intersection.getX(), intersection.getY(), intersection.getZ(), intersection.getX() + 1, intersection.getY() + 1, intersection.getZ() + 1), DgOneCongifConfig.oneconftodgcolor(TeleportSolverPage.solutionColor), partialTicks, false);
        }
    }
}
