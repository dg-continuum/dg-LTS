...
public class ActionClickSet extends AbstractAction {
    ...
    public void onPlayerInteract(DungeonRoom dungeonRoom, PlayerInteractEvent event, ActionRouteProperties actionRouteProperties) {
        ...
        for (OffsetPoint pt2 : target.getOffsetPointList()) {
            if (pt2.getVector3i(dungeonRoom).equals(event.pos) && predicate.test(event.entityLiving.getHeldItem())) {
                clicked = true;
            }
        }
    ...
    public void onRenderWorld(DungeonRoom dungeonRoom, float partialTicks, ActionRouteProperties actionRouteProperties, boolean flag) {
        ...
        for (OffsetPoint offsetPoint : target.getOffsetPointList()) {
            Vector3i pos = offsetPoint.getVector3i(dungeonRoom);
            xAcc += pos.x + 0.5f;
            yAcc += pos.y + 0.5f;
            zAcc += pos.z + 0.5f;
            RenderUtils.highlightBlock(offsetPoint.getVector3i(dungeonRoom), new Color(0, 255, 255, 50), partialTicks, true);
        }
